...
var BASE = {
    ...
    animateIndex: function (i) {
        var top = $(".intromain h2").eq(i).offset().top - 80;
        $('html,body').animate({scrollTop: top}, 200);
        var intro_li = $("#introsider li");
        intro_li.removeClass("on");
        intro_li.eq(i).addClass("on");
    },
    init: function () {
        var index = BASE.getQueryString("index");
        if (index) {
            BASE.animateIndex(index);
        }
        var intro_li = $("#introsider li");
        var topList = [];
        $("#introsider li").each(function (n) {
            var _self = $(this);
            var top = $(".intromain h2").eq(n).offset().top - 80;
        ...
        function check_i(top) {
            for (var i = 0; i < topList.length; i++) {
                if (top <= topList[0]) {
                    return 0;
                }
                if (top < topList[i] - 58 * i) {
                    return i - 1;
                }
                if (top > topList[topList.length - 1] - (topList.length - 1) * 77) {
        ...
        $(window).scroll(function () {
            ...
            var i = check_i(_top);
            intro_li.removeClass("on").eq(i).addClass("on");
        });
    }
}
